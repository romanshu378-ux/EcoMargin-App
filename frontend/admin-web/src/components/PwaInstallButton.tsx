...
export const PwaInstallButton: React.FC = () => {
  ...
  const handleInstall = async () => {
    if (!deferredPrompt) {
      alert('To install this Admin Web App: Tap Share -> "Add to Home Screen" or click the Install button in your browser address bar.');
      return;
    }
  ...
  if (isInstalled) {
    return (
      <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-emerald-500/10 text-emerald-400 text-xs font-semibold border border-emerald-500/20">
        <CheckCircle className="w-4 h-4" /> PWA Installed
      </span>
    );
  }
